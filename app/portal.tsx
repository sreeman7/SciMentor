...
export default function Portal({ user }: {
    ...
    // Optional agent access navigates the same workspace; mutations retain normal server authorization.
    useEffect(() => { const context = (document as Document & {
        ...
        void Promise.resolve(context.registerTool({ name: 'open_scimentor_section', description: 'Navigate to a section of the current SciMentor workspace. Does not create or send anything.', inputSchema: { type: 'object', properties: { section: { type: 'string', enum: Object.keys(titles) } }, required: ['section'], additionalProperties: false }, annotations: { readOnlyHint: true }, execute(input: unknown) { const value = (input as {
                section?: string;
            })?.section; if (!value || !(value in titles) || (!mentor && value === 'mentees'))
                throw new Error('Invalid section.'); go(value as View); return { section: value }; } }, { signal: lifecycle.signal })).catch(() => { });
    }
    catch { } return () => lifecycle.abort(); }, [mentor]);
 ...
 {modal?.type === 'invite' && <form className="form-stack" onSubmit={e => submit(e, 'invite')}><Field name="name" label="Mentee’s name" max={80}/><Field name="email" label="Mentee’s sign-in email" type="email" max={254} hint="Use the email they use to sign in with ChatGPT."/><button className="button" disabled={busy}>Create invite code <ArrowRight size={16}/></button></form>}

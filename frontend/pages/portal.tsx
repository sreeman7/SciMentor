...
function Navigation({ view, go, mentor, count, unreadCount }: {
    view: View;
    go: (v: View) => void;
    mentor: boolean;
    count: number;
    ...
    typeof CalendarDays
            ...
            View,
            typeof CalendarDays
        ]] : [])]; return <nav className="nav-list">{items.map(([v, Icon]) => <button key={v} className={`nav-item ${view === v ? 'active' : ''}`} onClick={() => { go(v); setOpenMobile(false); }} aria-current={view === v ? 'page' : undefined}><Icon size={19}/>{v === 'availability' && !mentor ? 'Book a meeting' : v === 'meetings' && !mentor ? 'My meetings' : titles[v]}{v === 'messages' && unreadCount > 0 && <span className="count" aria-label={`${unreadCount} unread messages`}>{unreadCount}</span>}{v === 'mentees' && count > 0 && <span className="count">{count}</span>}</button>)}</nav>; }
function RescheduleForm({ meeting, slots, busy, onSubmit }: {
    meeting: Meeting;
    slots: Slot[];
    busy: boolean;
    onSubmit: (event: FormEvent<HTMLFormElement>) => void;
}) {
    const available = slots.filter(s => s.id !== meeting.slot_id && !s.taken && canBook(s.start));
    ...
    return <form className="form-stack" onSubmit={onSubmit}>
        <div className="notice info mb-0"><strong>Current meeting</strong><br/>{fullDate(meeting.start)} · {clockTime(meeting.start)}–{clockTime(meeting.end)} · Edmonton time</div>
        {available.length ? <>
            <label className="field"><span>New meeting time (Edmonton)</span><select name="slotId" required value={slotId} onChange={e => {
                const slot = available.find(s => s.id === e.target.value);
                setSlotId(e.target.value);
                setFormat(slot?.format === 'both' ? meeting.format : slot?.format === 'in-person' ? 'in-person' : 'online');
            }}><option value="" disabled>Choose an available time</option>{available.map(s => <option key={s.id} value={s.id}>{fullDate(s.start)} · {clockTime(s.start)}–{clockTime(s.end)} · {s.format === 'both' ? 'Online or in-person' : s.format === 'online' ? 'Online' : 'In-person'}</option>)}</select></label>
            {selected && <><label className="field"><span>Meeting format</span><select name="format" value={format} onChange={e => setFormat(e.target.value as Meeting['format'])}>{(selected.format === 'both' ? ['online', 'in-person'] : [selected.format]).map(f => <option key={f} value={f}>{f === 'online' ? 'Online' : 'In-person'}</option>)}</select></label>
            {format === 'in-person' && <Field name="location" label="In-person meeting location" value={meeting.location} max={250} hint="Include the building, room, or a clear meeting point."/>}</>}
            <p className="muted">Your discussion topic will be kept. For an online meeting, your mentor will add a link for the new time.</p>
            <button className="button" disabled={busy || !selected}>Confirm new time <Check size={16}/></button>
        </> : <p className="muted">No other published times are available with 72 hours’ notice. Your current booking is unchanged. Check with your mentor about more availability.</p>}
    </form>;
}
export default function Portal({ user }: {
    ...
    const conversationId = mentor ? (person || mentees[0]?.user_id) : data?.member?.user_id;
    const conversationSuspended = mentor && mentees.find(p => p.user_id === conversationId)?.suspended_at != null;
    const conversationUnread = unread.filter(message => message.mentee_id === conversationId);
    ...
    const meetingRows = (list: Meeting[]) => list.map(m => <div className="list-row" key={m.id}><div className="row"><span className="avatar">{m.format === 'online' ? <Video size={18}/> : <MapPin size={18}/>}</span><div><h3>{mentor ? m.mentee_name : m.agenda}</h3><p>{fullDate(m.start)} · {clockTime(m.start)}–{clockTime(m.end)}</p><p>{m.format === 'in-person' ? m.location : 'Online meeting'}{m.status === 'cancelled' ? ' · Cancelled' : ''}</p>{mentor && <p className="pre-line">{m.agenda}</p>}</div></div><div className="actions">{m.status === 'confirmed' && m.format === 'online' && (m.link ? <a className="button secondary small" href={m.link} target="_blank" rel="noreferrer">Join meeting <ExternalLink size={14}/></a> : mentor ? <button className="button secondary small" onClick={() => open({ type: 'link', meeting: m })}>Add meeting link</button> : <span className="badge yellow">Link coming soon</span>)}{mentor && m.status === 'confirmed' && m.format === 'online' && m.link && <button className="text-link" onClick={() => open({ type: 'link', meeting: m })}>Edit meeting link</button>}{m.status === 'confirmed' && m.start > now && <button className="text-link" onClick={() => open({ type: 'reschedule', meeting: m })}>Reschedule</button>}{m.status === 'confirmed' && m.start > now && <button className="text-link" onClick={() => setConfirm({ action: 'cancel', id: m.id, title: 'Cancel this meeting?', description: 'The meeting will be marked cancelled for both of you and its time will be released.' })}>Cancel</button>}</div></div>);
    return <SidebarProvider><Sidebar><SidebarHeader><div className="brand"><GraduationCap /><span>SciMentor<span className="brand-sub">SCIENCE MENTORING</span></span></div></SidebarHeader><SidebarContent><div className="nav-label">YOUR WORKSPACE</div>{!suspended && <Navigation view={view} go={go} mentor={!!mentor} count={mentees.length} unreadCount={unread.length}/>}<div className="sidebar-note"><ShieldCheck size={22}/><strong>A little support goes a long way.</strong><p>Your questions and replies stay between you and your mentor.</p></div></SidebarContent><SidebarFooter><div className="profile"><span className="avatar">{(data?.member?.name || user.name || user.email).charAt(0).toUpperCase()}</span><div className="min-w-0 flex-1"><div className="truncate">{data?.member?.name || user.name || 'Your account'}</div><span>{data?.member ? (mentor ? 'Science mentor' : 'Science mentee') : 'Let’s get you settled'}</span></div><form action="/auth/signout" method="post"><button type="submit" aria-label="Sign out" title="Sign out"><LogOut size={17}/></button></form></div></SidebarFooter></Sidebar><main className="workspace"><header className="topbar"><div className="row"><SidebarTrigger /><span>{data?.group?.name || 'Your mentoring space'}</span></div><button className="text-link" disabled={loading || busy} onClick={load}>{loading ? 'Refreshing…' : 'Refresh'}</button><span className="pill"><LockKeyhole size={12}/> Private workspace</span></header><div className="page">
 ...
 {view === 'meetings' && <>{heading(mentor ? 'A little time, well spent.' : 'Your conversations, planned.', 'Meeting details are visible only to the mentor and the booked mentee.', !mentor ? <button className="button" onClick={() => go('availability')}><Plus size={17}/>Book a meeting</button> : undefined)}<div className="notice info"><Clock3 size={15} className="inline mr-2"/>All times are shown in {TIMEZONE}. Use Reschedule to choose a new published time at least 72 hours ahead. Your current booking stays confirmed until the change succeeds.</div><section className="panel">{data.meetings.length ? meetingRows(data.meetings) : <Empty title="No meetings yet">Your confirmed bookings will appear here, with the location or online meeting link.</Empty>}</section></>}
 {view === 'messages' && <>{heading('A safe place to ask.', 'Only you and the other person in this conversation can read these messages.')}<div className="panel chat-layout"><aside className="chat-list">{(mentor ? mentees : data.people.filter(p => p.role === 'mentor')).map(p => <button className={`chat-person ${person === p.user_id || (!person && p.user_id === (mentor ? mentees[0]?.user_id : data.people.find(p => p.role === 'mentor')?.user_id)) ? 'active' : ''}`} key={p.user_id} onClick={() => { setPerson(p.user_id); setReplyTo(null); setMessage(''); }}><span className="avatar">{p.name.charAt(0)}</span><span>{p.name}</span>{unread.some(m => m.mentee_id === (mentor ? p.user_id : data.member!.user_id)) && <span className="count">{unread.filter(m => m.mentee_id === (mentor ? p.user_id : data.member!.user_id)).length} unread</span>}</button>)}{mentor && !mentees.length && <p className="muted">Invite your mentees to start a conversation.</p>}</aside><section className="chat-main"><div className="section-head"><h2>{(mentor ? mentees : data.people.filter(p => p.role === 'mentor')).find(p => p.user_id === person)?.name || (mentor ? mentees[0]?.name : data.people.find(p => p.role === 'mentor')?.name) || 'Private messages'}</h2><span className="privacy-line"><LockKeyhole size={13}/> Only you two</span></div><div className="section-head"><span className="muted">{conversationUnread.length} unread messages</span>{conversationUnread.length > 0 && <button className="text-link" disabled={busy} onClick={() => run({ action: 'read-messages', ids: conversationUnread.slice(0, 100).map(m => m.id) })}>Mark {conversationUnread.length > 100 ? 'first 100' : 'conversation'} as read</button>}</div><div className="chat-messages">{data.messages.filter(m => m.mentee_id === (mentor ? (person || mentees[0]?.user_id) : data.member!.user_id)).map(m => <div key={m.id} className={`bubble ${m.sender_id === data.member!.user_id ? 'mine' : ''}`}>{m.announcement_title && <div className="badge mb-2">Re: {m.announcement_title}</div>}<div>{m.body}</div>{m.sender_id !== data.member!.user_id && m.read_at == null && <span className="badge yellow">Unread</span>}<small>{fullDate(m.created_at)} · {clockTime(m.created_at)}</small></div>)}{!data.messages.some(m => m.mentee_id === (mentor ? (person || mentees[0]?.user_id) : data.member!.user_id)) && <Empty icon={MessageSquare} title="Start with a question">Big or small, this is your space to talk.</Empty>}</div>{conversationSuspended && <div className="notice">This mentee’s access is suspended. Conversation history is available here; restore access from Mentees to send new messages.</div>}{(!mentor || mentees.length > 0) && !conversationSuspended && <form className="form-stack" onSubmit={async (e) => { e.preventDefault(); const r = await run({ action: 'message', menteeId: mentor ? (person || mentees[0]?.user_id) : data.member!.user_id, body: message, announcementId: replyTo?.id }); if (r) {
            setMessage('');
            setReplyTo(null);
 ...
 <Dialog open={!!modal} onOpenChange={v => { if (!v && !busy)
        setModal(null); }}><DialogContent className="max-h-[90vh] overflow-y-auto bg-white"><DialogHeader><DialogTitle>{modal?.type === 'availability' ? 'Make room for a conversation' : modal?.type === 'reschedule' ? 'Reschedule your meeting' : modal?.type === 'book' ? 'Book your meeting' : modal?.type === 'announce' ? 'Share an announcement' : modal?.type === 'invite' ? 'Invite a mentee' : modal?.type === 'link' ? 'Manage online meeting link' : 'Add to your knowledge base'}</DialogTitle><DialogDescription>{modal?.type === 'availability' ? 'Publish a time block for a specific date. We’ll split it into bookable meetings.' : modal?.type === 'reschedule' ? 'Choose a new time. Only you and the other meeting participant can see the details.' : modal?.type === 'book' ? 'Your booking details stay between you and your mentor.' : modal?.type === 'announce' ? 'Visible to your group. An email copy is queued for every joined mentee.' : modal?.type === 'invite' ? 'The code is tied to this email and can be used once within seven days.' : modal?.type === 'link' ? 'Only the booked mentee and you can see this link.' : 'Share an answer or a useful link with your mentees.'}</DialogDescription></DialogHeader>{error && <div className="error" role="alert">{error}</div>}
 ...
 {modal?.type === 'reschedule' && data && <RescheduleForm meeting={modal.meeting!} slots={data.slots} busy={busy} onSubmit={e => submit(e, 'reschedule', { id: modal.meeting!.id })}/>}
